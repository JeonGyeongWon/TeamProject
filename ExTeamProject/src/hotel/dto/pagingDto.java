...
public class pagingDto {
	...
	private void makePaging(){
		if(this.totalCount == 0) return;	//게시글이 없는경우
		if(this.pageNo == 0) this.setPageNo(1); //기본 페이지설정
		if(this.pageSize == 0) this.setPageSize(10); //기본 페이지수설정
		
		int finalPage = (totalCount + (pageSize -1)) /pageSize; //마지막 페이지
		if(this.pageNo > finalPage) this.setPageNo(finalPage); //현재 페이지번호가 마지막페이지보드 클경우 설정
		
		if(this.pageNo < 0 || this.pageNo > finalPage) this.pageNo=1;
		//현재 페이지 유효성 체크 어긋나면 기본페이지로 설정
		...
		this.setFinalPageNo(1);	//첫번쨰 페이지 번호
		
		if(isNowFirst){
			this.setPrevPageNo(1);	//이전 번호
		}else{
			this.setPrevPageNo(((pageNo -1) < 1 ? 1: (pageNo -1)));	//이전페이지 번호
		}
